...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['receiver_id']) && isset($_POST['message_content'])) {
    ...
    if ($receiver_id === false || $receiver_id <= 0) {
        ...
        if (!$selected_recipient) {
            ...
            if ($stmt->execute()) {
                header('Location: /shared/inbox.php?to=' . $selected_recipient_id . '&sent=1');
                exit();
            } else {
  ...
  <div class="page">
    ...
    <section class="dash">
      <section class="glass card search-card">
        ...
        <form method="POST" action="/shared/inbox.php<?php echo $selected_recipient_id ? '?to=' . e($selected_recipient_id) : ''; ?>" class="message-form">
