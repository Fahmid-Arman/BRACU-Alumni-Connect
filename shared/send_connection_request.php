...
function connection_request_return_path($path)
{
    ...
    if (preg_match('#^/alumni/alumni_list\.php(?:\?[A-Za-z0-9_=&%+\-\.]*)?$#', $path)) {
        return $path;
    }

    if (preg_match('#^/shared/view_alumni_profile\.php\?user_id=\d+(?:&[A-Za-z0-9_=&%+\-\.]*)?$#', $path)) {
        return $path;
    }
...
function connection_request_redirect_url($path, array $params)
{
    ...
    $base = $parts['path'] ?? '/student/student_search.php';
    $query = [];
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    redirect_to('/student/student_search.php');
}
